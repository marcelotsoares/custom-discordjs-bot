...
export class LevelController {
    ...
    async generateExpBasedOnRoleByMessageLength(messageLength: number): Promise<number> {
        if(messageLength <= 0) {
            throw new NotFoundException('required message length')
        }
        
        const generateExpBasedOnStringLength = messageLength * this.configExp.expPerCharacter;

        const generateExp = Math.floor(generateExpBasedOnStringLength * this.configExp.expRatioByDefaultRole) / 100
    ...
    async giveExpToUserByMessage({user, messageLength}: IUserLevel) {
        try {
            const userExpGenerated = await this.generateExpBasedOnRoleByMessageLength(messageLength)
            
            console.log(`[level:controller:generateXP] To user <${user.discordName}:${user.discordId}:${userExpGenerated} Exp Generated By Default Role (Ratio: ${this.configExp.expRatioByDefaultRole})>`)
